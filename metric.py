...
from sklearn.metrics import normalized_mutual_info_score, adjusted_rand_score, accuracy_score, f1_score, precision_score, recall_score
...
def inference(loader, model, device, view, data_size, class_num):
    ...
    for step, (xs, y, _) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            cs = model.compute_centers(hs, qs)
            q = sum(qs)/view
        for v in range(view):
            hs[v] = hs[v].detach()
            zs[v] = zs[v].detach()
            ...
            Cs[v].extend(cs[v].cpu().detach().numpy())
    ...
    for v in range(view):
        ...
        Zs[v] = np.array(Zs[v])
        Cs[v] = np.array(Cs[v])
        Prototype[v] = np.array(Prototype[v])
        
        pred_vectors[v] = np.array(pred_vectors[v])

    return total_pred, pred_vectors, Hs, labels_vector, Zs, Cs, Prototype
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    total_pred, pred_vectors, high_level_vectors, labels_vector, low_level_vectors, Centers, prototype_feature = inference(test_loader, model, device, view, data_size, class_num)
    ...
    print("Clustering results on cluster assignments of total view:")
    print('ACC = {:.4f} NMI = {:.4f} PUR = {:.4f} ARI={:.4f}'.format(acc, nmi, pur, ari))
    return nmi, ari, acc, pur, total_pred, pred_vectors, high_level_vectors, labels_vector, low_level_vectors, Centers, prototype_feature
